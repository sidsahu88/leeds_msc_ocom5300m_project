...
class PrimaryCapsLayer(nn.Module):
    def __init__(self, in_channels=256, n_out_caps=32, out_caps_dim=8, kernel_size=9, stride=2, padding='valid', *args, **kwargs):
        ...
        #     [nn.Conv2d(in_channels=in_channels, out_channels=n_out_caps, kernel_size=kernel_size, stride=stride, padding=padding)
    ...
    def forward(self, x):
        ...
        batch_size = x.size(0)
        # caps_output = torch.stack([capsule(x) for capsule in self.primary_caps], dim=-1)    # caps_output shape: (batch_size, n_out_caps, conv_out_dim1, conv_out_dim2, out_caps_dim)
...
class ClassCapsLayer(nn.Module):
    def __init__(self, n_class, n_in_caps, in_caps_dim=8, out_caps_dim=16, *args, **kwargs):
        ...
        self.weights = nn.Parameter(torch.randn(n_in_caps, n_class, out_caps_dim, in_caps_dim))

    def forward(self, u):
        batch_size = u.size(0)
        W = torch.stack([self.weights] * batch_size, dim=0)         # W Shape: (batch_size, n_in_caps, n_class, out_caps_dim, in_caps_dim)
        # print("W shape: ", W.size())
        
        u = torch.stack([u] * self.n_class, dim=2).unsqueeze(-1)    # u Shape: (batch_size, n_in_caps, n_class, in_caps_dim, 1)
        # print("u shape: ", u.size())
        return torch.matmul(W, u)                                   # return Shape: (batch_size, n_in_caps, n_class, out_caps_dim, 1)
...
class BaseCapsuleNetwork(nn.Module):
    def __init__(self, in_img_c, in_img_h, in_img_w, n_class, *args, **kwargs):
        ...
        self.primary_caps = PrimaryCapsLayer(n_out_caps=32, kernel_size=9, stride=2)
        ...
        self.class_caps = ClassCapsLayer(n_class=n_class, n_in_caps=n_prim_caps)
